...
def array_sort_by_last_letter_of_word(array)
  reversed = reverse_every_element_in_array(array).sort
  reverse_every_element_in_array(reversed)
end
...
def convert_array_to_a_hash(array)
  ...
  array.each_with_index do |element, index|
    result[element] = array[index+1] if index%2 == 0
  end
  result
end
...
def add_together_keys_and_values(hash)
  total_of_array(hash.keys) + total_of_array(hash.values)
end
...
def titleize_a_string(string)
  ...
  string.split(" ").each do |word|
    word.capitalize! unless to_ignore.include?(word)
  end.join(" ")
end
